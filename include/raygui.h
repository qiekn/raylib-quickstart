 ...
 *library 1.0 (07-Jun-2016) Converted to header-only by Ramon Santamaria 0.9 (07-Mar-2016) Reviewed and tested by Albert
 *Martos, Ian Eito, Sergio Martinez and Ramon Santamaria 0.8 (27-Aug-2015) Initial release. Implemented by Kevin Gato,
 *Daniel Nicolás and Ramon Santamaria
 *
 *   DEPENDENCIES:
...
#define RAYGUI_GROUPBOX_LINE_THICK 1
  ...
  //--------------------------------------------------------------------
  GuiDrawRectangle(
      RAYGUI_CLITERAL(Rectangle){bounds.x, bounds.y, RAYGUI_GROUPBOX_LINE_THICK, bounds.height}, 0, BLANK,
  ...
  GuiDrawRectangle(
      ...
      BLANK, GetColor(GuiGetStyle(DEFAULT, (state == STATE_DISABLED) ? (int)BORDER_COLOR_DISABLED : (int)LINE_COLOR)));
  GuiDrawRectangle(
...
#define RAYGUI_LINE_TEXT_PADDING 4
  ...
  if (text == NULL)
    ...
    GuiDrawRectangle(
        RAYGUI_CLITERAL(Rectangle){bounds.x + 12 + textBounds.width + 4, bounds.y + bounds.height / 2,
      ...
      (wrapMode == TEXT_WRAP_NONE))            // No wrap mode
  {
    ...
    if (editMode) {
      ...
      // Delete single codepoint from text, before current cursor position
               ...
               (IsKeyPressed(KEY_BACKSPACE) || (IsKeyDown(KEY_BACKSPACE) && autoCursorShouldTrigger))) {
...
int GuiSpinner(Rectangle bounds, const char *text, int *value, int minValue, int maxValue, bool editMode) {
  ...
  Rectangle valueBoxBounds = {
      ...
      bounds.width - 2 * (GuiGetStyle(VALUEBOX, SPINNER_BUTTON_WIDTH) + GuiGetStyle(VALUEBOX, SPINNER_BUTTON_SPACING)),
      bounds.height};
  Rectangle leftButtonBound = {(float)bounds.x, (float)bounds.y, (float)GuiGetStyle(VALUEBOX, SPINNER_BUTTON_WIDTH),
                               (float)bounds.height};
    ...
    if (guiControlExclusiveMode)  // Allows to keep dragging outside of bounds
    {
      if (IsMouseButtonDown(MOUSE_LEFT_BUTTON)) {
        if (CHECK_BOUNDS_ID(bounds, guiControlExclusiveRec)) {
          ...
          *value =
              (maxValue - minValue) * ((mousePoint.x - bounds.x - sliderWidth / 2) / (bounds.width - sliderWidth)) +
              minValue;
        }
      } else {
...
int GuiProgressBar(Rectangle bounds, const char *textLeft, const char *textRight, float *value, float minValue,
  ...
  if ((state != STATE_DISABLED))
    progress.width =
        ((float)*value / (maxValue - minValue)) * (bounds.width - 2 * GuiGetStyle(PROGRESSBAR, BORDER_WIDTH));
  //--------------------------------------------------------------------
  ...
  if (state == STATE_DISABLED) {
    ...
    if (*value > minValue) {
      ...
      GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle){bounds.x, bounds.y + 1, (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH),
                                                  bounds.height - 2},
                       0, BLANK, GetColor(GuiGetStyle(PROGRESSBAR, BORDER_COLOR_FOCUSED)));
    ...
    if (*value >= maxValue)
      GuiDrawRectangle(
          RAYGUI_CLITERAL(Rectangle){bounds.x + progress.width + (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH),
      ...
      GuiDrawRectangle(
          RAYGUI_CLITERAL(Rectangle){
              bounds.x + (int)progress.width + (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH), bounds.y,
      ...
      GuiDrawRectangle(
          ...
          0, BLANK, GetColor(GuiGetStyle(PROGRESSBAR, BORDER_COLOR_NORMAL)));
      GuiDrawRectangle(
          RAYGUI_CLITERAL(Rectangle){bounds.x + bounds.width - (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH), bounds.y,
                                     (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH),
                                     bounds.height + GuiGetStyle(PROGRESSBAR, BORDER_WIDTH) - 1},
          0, BLANK, GetColor(GuiGetStyle(PROGRESSBAR, BORDER_COLOR_NORMAL)));
    }
...
int GuiColorPicker(Rectangle bounds, const char *text, Color *color) {
  ...
  // NOTE: this conversion can cause low hue-resolution, if the r, g and b value are very similar, which causes the hue
  // bar to shift around when only the GuiColorPanel is used
  Vector3 hsv =
        ...
        switch (buffer[0]) {
          ...
          case 'f': {
            ...
            if (fontFileName[0] != '\0') {
              ...
              if (codepointCount > 0)
                font = LoadFontEx(TextFormat("%s/%s", GetDirectoryPath(fileName), fontFileName), fontSize, codepoints,
                                  codepointCount);
              else
                font = LoadFontEx(TextFormat("%s/%s", GetDirectoryPath(fileName), fontFileName), fontSize, NULL,
...
void GuiLoadStyleDefault(void) {
  ...
  if (guiFont.texture.id != GetFontDefault().texture.id) {
    ...
    SetShapesTexture(guiFont.texture, RAYGUI_CLITERAL(Rectangle){whiteChar.x + 1, whiteChar.y + 1, whiteChar.width - 2,
                                                                 whiteChar.height - 2});
  }
}
    ...
    for (int c = 0, codepointSize = 0; c < lineSize; c += codepointSize) {
      ...
      if (codepoint == '\n')
          ...
          if (wrapMode == TEXT_WRAP_NONE) {
            ...
            if (textSizeX > textBounds.width) {
              if (textOffsetX <= (textBounds.width - glyphWidth - textBoundsWidthOffset - ellipsisWidth)) {
                DrawTextCodepoint(
                    guiFont, codepoint,
                    RAYGUI_CLITERAL(Vector2){textBoundsPosition.x + textOffsetX, textBoundsPosition.y + textOffsetY},
        ...
        if (guiFont.glyphs[index].advanceX == 0)
          ...
          textOffsetX +=
              ((float)guiFont.glyphs[index].advanceX * scaleFactor + (float)GuiGetStyle(DEFAULT, TEXT_SPACING));
      }
    }
